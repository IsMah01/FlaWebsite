...
app.get("/api/admin/profile-image/:fileName", async (c) => {
  const adminId = await getInternalAdminId(c.req.raw);
  const fileName = c.req.param("fileName");
  if (!adminId) return c.json({ error: "Forbidden" }, 403);
  if (!/^admin-\d+-[a-f0-9-]+\.(jpg|png)$/i.test(fileName)) return c.json({ error: "Invalid file name" }, 400);
  try {
    const [rows] = await getSqlPool().query<any[]>(`SELECT id FROM admin_users WHERE id=? AND profileImageRef=? LIMIT 1`, [adminId, fileName]);
    if (!rows[0]) return c.json({ error: "Not found" }, 404);
    const data = await readFile(path.resolve(process.cwd(), "storage", "private", "uploads", "admin-profiles", fileName));
