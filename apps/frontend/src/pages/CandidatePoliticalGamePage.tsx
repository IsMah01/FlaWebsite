...
export default function CandidatePoliticalGamePage() {
  ...
  const role = trpc.politicalGame.myAssignment.useQuery(undefined, { enabled: opened, retry: false });
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top,#245c54,#071713)] p-4" lang="ar" dir="rtl"><Navbar/><main className="mx-auto mt-20 w-full max-w-2xl rounded-3xl border border-white/15 bg-white p-6 text-center shadow-2xl sm:p-10">
    ...
    {!opened ? <section className="mt-7 rounded-2xl border-2 border-amber-300 bg-amber-50 p-6"><ShieldAlert className="mx-auto h-12 w-12 text-amber-600"/><h2 className="mt-4 text-2xl font-black text-amber-950">تنبيه سري</h2><p className="mt-3 leading-8 text-amber-900">تأكد أنك وحدك وأن لا أحد ينظر إلى شاشتك. دورك ومعلومات مهمتك سرية ولا يجب أن يراها أي شخص آخر.</p><Button className="mt-6 h-12 bg-violet-700 px-8 text-lg font-black hover:bg-violet-800" onClick={() => setOpened(true)}><LockKeyhole className="ml-2 h-5 w-5"/>فتح دوري السري</Button></section> : role.isLoading ? <p className="mt-8">جارٍ تحميل الدور…</p> : role.isError ? <p className="mt-8 rounded-xl bg-red-50 p-4 font-bold text-red-700">يرجى تسجيل الدخول بحساب المشارك المؤكد.</p> : !role.data ? <p className="mt-8 rounded-xl bg-amber-50 p-5 font-bold text-amber-800">لم يتم نشر دورك بعد.</p> : <div className="mt-7 space-y-5">
      <div className={`rounded-2xl p-6 ${role.data.isSpy ? "bg-red-50 text-red-900" : "bg-emerald-50 text-emerald-900"}`}><ShieldAlert className="mx-auto h-10 w-10"/><p className="mt-3 text-2xl font-black">{role.data.isSpy ? "أنت جاسوس" : "أنت لست جاسوساً"}</p>{role.data.isSpy ? <div className="mt-4 space-y-2"><p>الدور المزيف: <strong>{role.data.displayedRole}</strong></p><p>بلد المهمة الحقيقي: <strong>{role.data.spyCountry}</strong></p><p>بلد التغطية المزيف: <strong>{role.data.fakeCountry}</strong></p>{role.data.contact ? <p>الشخص الذي يجب التواصل معه: <strong>{role.data.contact.firstName} {role.data.contact.lastName}</strong><br/><a dir="ltr" href={`mailto:${role.data.contact.email}`} className="underline">{role.data.contact.email}</a>{role.data.contact.phoneNumber ? <><br/><a dir="ltr" href={`tel:${role.data.contact.phoneNumber}`} className="underline">{role.data.contact.phoneNumber}</a></>:null}</p>:null}</div>:null}</div>
      {role.data.isIntelligencePresident ? <section className="rounded-2xl border-2 border-violet-200 bg-violet-50 p-5 text-right"><h2 className="flex items-center gap-2 text-xl font-black text-violet-950"><UserRound className="h-6 w-6"/>رئاسة الاستخبارات</h2><p className="mt-2 text-violet-800">الجواسيس الذين يجب عليهم التواصل معك:</p><div className="mt-4 space-y-3">{role.data.spies.map((spy,i)=><div key={i} className="rounded-xl bg-white p-3"><strong>{spy.firstName} {spy.lastName}</strong><div className="mt-1 text-sm text-slate-600">الدور المزيف: <strong>{spy.displayedRole}</strong><br/>البلد الحقيقي: <strong>{spy.spyCountry}</strong> · بلد التغطية: <strong>{spy.fakeCountry}</strong></div></div>)}</div></section>:null}
