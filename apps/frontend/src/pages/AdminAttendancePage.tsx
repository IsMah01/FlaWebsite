...
export default function AdminAttendancePage() {
  ...
  return <div className="min-h-screen bg-slate-50 p-4 md:p-8" lang="ar" dir="rtl"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-center gap-3"><span className="font-black text-slate-700">الحصص المفتوحة الآن:</span>{openSessions.length ? openSessions.map((session) => <Button key={session.id} size="sm" variant={qr?.sessionId === session.id ? "default" : "outline"} className={qr?.sessionId === session.id ? "bg-[#4A9B8E] hover:bg-[#3D7A6F]" : ""} onClick={() => showQr(session.id, session.title, session.token)}><QrCode className="ml-1 h-4 w-4" />{session.title}</Button>) : <span className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-500">لا توجد حصة مفتوحة</span>}</div></section>
    ...
    {attendanceSession ? <section className="rounded-3xl border bg-white p-6 shadow-lg"><div className="flex items-center justify-between gap-3"><div><h2 className="text-2xl font-black">متابعة الحضور والغياب</h2><p className="mt-1 text-slate-500">{attendanceSession.title}</p></div><Button variant="ghost" onClick={() => setAttendanceSession(null)}><X className="h-5 w-5" /></Button></div>{attendance.isLoading ? <p className="py-10 text-center text-slate-500">جارٍ تحميل اللوائح…</p> : attendance.data ? <div className="mt-5 grid gap-6 xl:grid-cols-2"><CandidateAttendanceList title={`الحاضرون (${attendance.data.present.length})`} candidates={attendance.data.present} present /><CandidateAttendanceList title={`غير المسجلين بعد (${attendance.data.absent.length})`} candidates={attendance.data.absent} /></div> : <p className="mt-5 rounded-xl bg-red-50 p-6 text-center text-red-700">تعذر تحميل اللوائح.</p>}</section> : null}
    ...
    <div className="space-y-6">{programmeDays.map((day, dayIndex) => <section key={day.day} className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="flex items-center justify-between bg-slate-100 p-4"><h2 className="text-xl font-black">{day.day}</h2><span className="text-sm text-slate-500">{day.events.length} حصص</span></div><div className="divide-y">{day.events.map((event, eventIndex) => {
      ...
      return <div key={stableKey} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-start gap-3"><span className={`mt-1 h-3 w-3 shrink-0 rounded-full ${saved?.isOpen ? "bg-emerald-500" : "bg-slate-300"}`} /><div><p className="font-black">{event.title}</p><p className="mt-1 text-sm text-slate-500" dir="ltr">{event.time}</p></div></div><div className="flex flex-wrap items-center gap-2">{saved ? <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700"><Users className="h-3.5 w-3.5" />{saved.attendanceCount} حاضر</span> : null}{saved ? <Button size="sm" variant="outline" onClick={() => showQr(saved.id, event.title, saved.token)}><QrCode className="ml-1 h-4 w-4" />QR</Button> : null}{saved ? <Button size="sm" variant="outline" onClick={() => setAttendanceSession({ id: saved.id, title: event.title })}><Eye className="ml-1 h-4 w-4" />اللائحة</Button> : null}{saved?.isOpen ? <Button size="sm" variant="destructive" disabled={busy} onClick={() => close.mutate({ id: saved.id })}><Square className="ml-1 h-4 w-4" />إغلاق</Button> : <Button size="sm" className="bg-emerald-700 hover:bg-emerald-800" disabled={busy} onClick={() => open.mutate({ scheduleKey: saved?.scheduleKey ?? stableKey, title: event.title, dayNumber: dayIndex + 1, timeLabel: event.time })}><CheckCircle2 className="ml-1 h-4 w-4" />{open.isPending ? "جارٍ الفتح…" : "فتح"}</Button>}</div></div>;
    })}</div></section>)}</div>
  </div></div>;
}

function CandidateAttendanceList({ title, candidates, present = false }: { title: string; candidates: Array<{ id: number; firstName: string; lastName: string; email: string; phoneNumber: string; checkedInAt: unknown }>; present?: boolean }) {
  return <div className={`overflow-hidden rounded-2xl border ${present ? "border-emerald-200" : "border-amber-200"}`}><h3 className={`px-4 py-3 font-black ${present ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}>{title}</h3>{candidates.length ? <div className="max-h-[480px] overflow-auto"><table className="w-full text-right text-sm"><thead className="sticky top-0 bg-white"><tr className="border-b"><th className="p-3">الاسم الكامل</th><th className="p-3">البريد الإلكتروني</th>{present ? <th className="p-3">وقت الحضور</th> : null}</tr></thead><tbody>{candidates.map((candidate) => <tr key={candidate.id} className="border-b last:border-0"><td className="p-3 font-bold">{candidate.firstName} {candidate.lastName}</td><td className="p-3" dir="ltr">{candidate.email}</td>{present ? <td className="whitespace-nowrap p-3" dir="ltr">{new Date(candidate.checkedInAt as string).toLocaleString("fr-MA")}</td> : null}</tr>)}</tbody></table></div> : <p className="p-6 text-center text-slate-500">لا يوجد أي اسم في هذه اللائحة.</p>}</div>;
}
