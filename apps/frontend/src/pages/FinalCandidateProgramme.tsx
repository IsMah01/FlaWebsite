...
const permanentCandidateFormUrl = "https://docs.google.com/forms/d/e/1FAIpQLSfmDbHNI09ckcqQujpP12YJwuRSPac38DtMsEt7P55wZ91tsw/viewform";
...
export const programmeDays: ProgrammeDay[] = [
  ...
  { day: "اليوم السابع", events: [
    ...
    { time: "14:30 — 16:00", title: "ورشة نقاش", detail: "المسؤوليات الأسرية والمسار الشخصي: أي نموذج في عالم اليوم؟", period: "afternoon" },
    { time: "16:00 — 17:30", title: "تكوين اللعبة السياسية", period: "afternoon" },
  ...
  { day: "اليوم الثامن", events: [
    ...
    { time: "16:00 — 17:30", title: "حصة المشاريع", period: "afternoon" },
  ...
  { day: "اليوم العاشر", events: [
    ...
    { time: "14:30 — 16:00", title: "ورشة نقاش", detail: "الهوية الإسلامية في عالم متغير: كيف يواجه الشباب تحديات العولمة والقيم الوافدة؟", period: "afternoon" },
    { time: "16:00 — 17:30", title: "التحضير للحفل الختامي", period: "afternoon" },
...
const periodStyle = {
  ...
  afternoon: { label: "الفترة الزوالية", Icon: Sun, className: "border-orange-200 bg-orange-50 text-orange-950", icon: "text-orange-600" },
...
export default function FinalCandidateProgramme() {
  ...
  return <div className="candidate-orange-theme min-h-screen bg-[#F4F8F7]" lang="ar" dir="rtl"><Navbar /><main className="mx-auto max-w-7xl px-4 pb-12 pt-24 sm:px-6">
    {access.isLoading ? <div className="flex min-h-[60vh] items-center justify-center"><div className="h-12 w-12 animate-spin rounded-full border-4 border-[#4A9B8E] border-t-transparent" /></div> : access.isError ? <section className="mx-auto mt-12 max-w-lg rounded-3xl border bg-white p-8 text-center shadow-sm"><LockKeyhole className="mx-auto h-14 w-14 text-amber-600" /><h1 className="mt-5 text-2xl font-black text-slate-900">فضاء خاص بالمشاركين المؤكدين</h1><p className="mt-3 leading-8 text-slate-600">{access.error.data?.code === "UNAUTHORIZED" ? "يرجى تسجيل الدخول بالحساب الذي استعملتموه لتأكيد مشاركتكم النهائية." : access.error.message}</p>{access.error.data?.code === "UNAUTHORIZED" ? <Link to="/signin?redirect=/espace-candidat-final"><Button className="mt-6 bg-[#4A9B8E] hover:bg-[#3D7A6F]">تسجيل الدخول</Button></Link> : <Link to="/"><Button className="mt-6" variant="outline">العودة إلى الرئيسية</Button></Link>}</section> : <>
      <header className="relative overflow-hidden rounded-[2rem] bg-[linear-gradient(125deg,#102f2b_0%,#1f5b52_48%,#4A9B8E_100%)] text-white shadow-[0_24px_70px_-30px_rgba(23,63,57,0.75)]">
        <div className="relative z-10 mx-auto mt-6 w-fit sm:mt-9"><img src="/images/edition-18-logo-color.png" alt="أكاديمية أطر الغد - الدورة 18 - دورة الأثر" className="h-20 w-auto object-contain drop-shadow-lg sm:h-28" /></div>
        <div className="absolute -left-20 -top-24 h-80 w-80 rounded-full bg-[#F28C18]/25 blur-3xl" /><div className="absolute -bottom-28 right-1/3 h-72 w-72 rounded-full bg-white/10 blur-3xl" /><div className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: "radial-gradient(circle at 2px 2px, white 1px, transparent 0)", backgroundSize: "28px 28px" }} />
...
function DailyFormsCard({ accountEmail }: { accountEmail: string }) {
  ...
  const pendingForms = daily.data?.dailyForms ?? [];
