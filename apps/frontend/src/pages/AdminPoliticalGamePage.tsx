...
import { ArrowLeft, Eye, EyeOff, Search, Shield, UserRound, X } from "lucide-react";
...
export default function AdminPoliticalGamePage() {
  ...
  const mutation = trpc.politicalGame.saveAll.useMutation({
    onSuccess: async r => { setPublishReviewOpen(false); toast.success(`تم نشر ${r.saved} دوراً في حسابات المشاركين.`); await overview.refetch(); },
  ...
  const visibility = trpc.politicalGame.setVisibility.useMutation({
    onSuccess: async result => { toast.success(result.visible ? "تم تفعيل عرض الأدوار للجميع." : "تم إيقاف عرض الأدوار للجميع."); await overview.refetch(); },
    onError: error => toast.error(error.message),
  });
  const people = overview.data?.candidates ?? [];
  const spiesForReview = drafts.filter(draft => draft.isSpy).map(draft => ({ draft, person: people.find(person => person.id === draft.candidateId), contact: people.find(person => person.id === draft.contactCandidateId) })).filter(item => item.person);
  const reviewIsValid = spiesForReview.every(({draft}) => draft.displayedRole && draft.spyCountry && draft.fakeCountry && draft.spyCountry !== draft.fakeCountry && draft.contactCandidateId);
  ...
  return <div className="min-h-screen bg-slate-50 p-4 md:p-8" lang="ar" dir="rtl">
    <main className="mx-auto max-w-7xl space-y-6">
      <header className="rounded-3xl bg-[linear-gradient(125deg,#102f2b,#4A9B8E)] p-6 text-white shadow-xl md:p-8">
        ...
        <div className="mt-6 flex flex-wrap items-end justify-between gap-4"><div><p className="font-bold text-emerald-100">إعدادات سرية</p><h1 className="mt-1 text-3xl font-black">اللعبة السياسية</h1><p className="mt-2 text-white/75">اختر كل جاسوس وبلده الحقيقي وبلد التغطية والدور المزيف والشخص الذي سيتواصل معه.</p></div><div className="flex flex-wrap gap-2"><Button className={overview.data?.rolesVisible ? "bg-red-600 text-white hover:bg-red-700" : "bg-emerald-500 text-white hover:bg-emerald-600"} disabled={visibility.isPending} onClick={() => visibility.mutate({ visible: !overview.data?.rolesVisible })}>{overview.data?.rolesVisible ? <EyeOff className="ml-2 h-4 w-4"/> : <Eye className="ml-2 h-4 w-4"/>}{visibility.isPending ? "جارٍ التحديث…" : overview.data?.rolesVisible ? "إيقاف عرض الأدوار للجميع" : "تفعيل عرض الأدوار للجميع"}</Button><Button className="bg-amber-400 text-slate-950 hover:bg-amber-300" disabled={mutation.isPending || !drafts.length} onClick={() => setPublishReviewOpen(true)}><Shield className="ml-2 h-4 w-4"/>{mutation.isPending ? "جارٍ النشر…" : "مراجعة ونشر الأدوار"}</Button></div></div>
      </header>
      ...
      <section className="grid gap-4">
        {filtered.map(person => { const d = drafts.find(x => x.candidateId === person.id); const published = overview.data?.assignments.find(a => a.id === person.id); if (!d) return null; return <article key={person.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${d.isSpy ? "border-red-300" : d.isIntelligencePresident ? "border-violet-300" : ""}`}>
          <div className="flex flex-col gap-4 border-b border-slate-100 pb-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0"><p className="font-black text-slate-900">{person.firstName} {person.lastName}</p><p className="truncate text-sm text-slate-500">{person.email}</p>{published?.revealedAt ? <p className="mt-2 inline-flex rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">تمت مشاهدة الدور · {new Date(published.revealedAt).toLocaleString("ar-MA")}</p> : <p className="mt-2 inline-flex rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">لم تتم مشاهدة الدور بعد</p>}</div>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
              <label className="flex cursor-pointer items-center gap-2 whitespace-nowrap font-bold text-red-700"><input type="checkbox" checked={d.isSpy} onChange={e => {if(!e.target.checked)setContactSearch(current=>({...current,[person.id]:""}));update(person.id,{isSpy:e.target.checked, displayedRole:e.target.checked ? d.displayedRole : "", spyCountry:e.target.checked ? d.spyCountry : "", fakeCountry:e.target.checked ? d.fakeCountry : "", contactCandidateId:e.target.checked ? d.contactCandidateId : null});}}/><Shield className="h-4 w-4"/>جاسوس</label>
              <label className="flex cursor-pointer items-center gap-2 whitespace-nowrap font-bold text-violet-700"><input type="checkbox" checked={testPresidentIds.has(person.id)} onChange={e=>setTestPresidentIds(current=>{const next=new Set(current);if(e.target.checked)next.add(person.id);else next.delete(person.id);return next;})}/><UserRound className="h-4 w-4"/>اختبار رئاسة الاستخبارات</label>
            </div>
          </div>
      ...
      {publishReviewOpen ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" onClick={()=>setPublishReviewOpen(false)}><section className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onClick={event=>event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-red-700">مراجعة نهائية قبل النشر</p><h2 className="mt-1 text-2xl font-black">قائمة الجواسيس ({spiesForReview.length})</h2><p className="mt-2 text-slate-600">سيتم نشر دور غير جاسوس تلقائياً لبقية المشاركين وعددهم {Math.max(0, drafts.length-spiesForReview.length)}.</p></div><Button size="icon" variant="ghost" onClick={()=>setPublishReviewOpen(false)}><X className="h-5 w-5"/></Button></div><div className="mt-6 grid gap-3 md:grid-cols-2">{spiesForReview.map(({draft,person,contact})=><article key={draft.candidateId} className="rounded-2xl border border-red-200 bg-red-50 p-4"><p className="text-lg font-black text-red-950">{person!.firstName} {person!.lastName}</p><p className="text-sm text-red-800" dir="ltr">{person!.email}</p><div className="mt-3 text-sm leading-7 text-slate-700">الدور المزيف: <strong>{draft.displayedRole||"غير محدد"}</strong><br/>البلد الحقيقي: <strong>{draft.spyCountry||"غير محدد"}</strong><br/>بلد التغطية: <strong>{draft.fakeCountry||"غير محدد"}</strong><br/>الشخص الذي سيتواصل معه: <strong>{contact ? `${contact.firstName} ${contact.lastName}` : "غير محدد"}</strong>{contact ? <><br/><span dir="ltr">{contact.email}</span>{contact.phoneNumber ? <><br/><span dir="ltr">{contact.phoneNumber}</span></> : null}</> : null}</div></article>)}</div>{!spiesForReview.length ? <p className="mt-6 rounded-xl bg-amber-50 p-4 font-bold text-amber-800">لم يتم اختيار أي جاسوس.</p> : null}{!reviewIsValid ? <p className="mt-4 rounded-xl bg-red-50 p-4 font-bold text-red-700">أكمل جميع معلومات الجواسيس وصحح البلدان قبل النشر.</p> : null}<div className="mt-6 flex flex-wrap justify-end gap-3"><Button variant="outline" onClick={()=>setPublishReviewOpen(false)}>العودة والتعديل</Button><Button className="bg-emerald-600 text-white hover:bg-emerald-700" disabled={mutation.isPending || !reviewIsValid} onClick={()=>mutation.mutate({assignments:drafts})}><Shield className="ml-2 h-4 w-4"/>{mutation.isPending ? "جارٍ النشر…" : "تأكيد ونشر الأدوار للجميع"}</Button></div></section></div> : null}
      {presidentPreview ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4" onClick={()=>setPresidentPreviewId(null)}><section className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onClick={event=>event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold text-violet-700">معاينة الحساب</p><h2 className="mt-1 flex items-center gap-2 text-2xl font-black"><UserRound className="h-6 w-6"/>رئاسة الاستخبارات</h2><p className="mt-2 text-slate-600">سيشاهد {presidentPreview.firstName} {presidentPreview.lastName} الجواسيس الآتين فقط.</p><p className="mt-2 rounded-lg bg-amber-50 p-2 text-sm font-bold text-amber-800">هذه معاينة للإعدادات الحالية قبل النشر.</p></div><Button size="icon" variant="ghost" onClick={()=>setPresidentPreviewId(null)}><X className="h-5 w-5"/></Button></div><div className="mt-6 space-y-3">{presidentSpies.map(({draft,person})=><article key={draft.candidateId} className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><p className="font-black">{person!.firstName} {person!.lastName}</p><p className="mt-2 text-sm text-slate-700">الدور المزيف: <strong>{draft.displayedRole||"غير محدد"}</strong><br/>البلد الحقيقي: <strong>{draft.spyCountry||"غير محدد"}</strong><br/>بلد التغطية: <strong>{draft.fakeCountry||"غير محدد"}</strong><br/>البريد الإلكتروني: <strong dir="ltr">{person!.email}</strong>{person!.phoneNumber ? <><br/>رقم الهاتف: <strong dir="ltr">{person!.phoneNumber}</strong></> : null}</p></article>)}</div></section></div>:null}
