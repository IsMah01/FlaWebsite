    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8" lang="fr">
      <div className="mx-auto max-w-[1500px] space-y-6">
        ...
        <section className="rounded-2xl border bg-white p-6 shadow-sm">
          ...
          {pendingFileName ? <div className="mt-4 rounded-xl border bg-slate-50 p-4 text-sm"><p className="font-semibold">{pendingFileName}</p><p className="mt-1 text-slate-600">{matchedPending.length} correspondance(s) · {unknownPending.length} sans correspondance · {allCandidates.length - matchedPending.length} futur(s) candidat(s) en liste d’attente</p>{unknownPending.length ? <p className="mt-2 break-words text-red-700">Sans correspondance : {unknownPending.slice(0, 15).join(", ")}{unknownPending.length > 15 ? "…" : ""}</p> : null}<Button className="mt-3" size="sm" variant="outline" onClick={() => { setPendingEmails([]); setPendingFileName(""); }}>Annuler cet import</Button></div> : null}
        </section>
