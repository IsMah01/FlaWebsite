...
export default function FinalCandidateConfirmation() {
  ...
  return <div className="min-h-screen bg-[#F8FAF9]" lang="fr" dir="ltr">
    <Navbar />
    <main className="flex min-h-[calc(100dvh-64px)] items-center justify-center px-3 pb-8 pt-20 sm:px-4 sm:pb-12 sm:pt-24">
      <section className="w-full max-w-lg rounded-2xl border bg-white p-4 shadow-sm sm:p-6 md:p-8">
        ...
        {step === "email" ? <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); lookup.mutate({ email }); }}>
          <div className="rounded-xl border border-blue-100 bg-blue-50 p-4 text-sm leading-6 text-blue-900"><div className="flex items-center gap-2 font-bold"><Search className="h-4 w-4" />Recherchez votre adresse e-mail</div><p className="mt-1 h-12 text-base">Saisissez l’adresse utilisée sur le site. Nous vérifierons automatiquement si vous avez déjà un compte.</p></div>
          <div><Label htmlFor="final-email">Adresse e-mail</Label><Input id="final-email" type="email" required autoFocus value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="vous@exemple.com" className="mt-1 h-12 text-base" /></div>
        ...
        {step === "existing" ? <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); confirmExisting.mutate({ email, password: form.password }); }}>
          <div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-sm leading-6 text-emerald-900"><div className="flex items-center gap-2 font-bold"><CheckCircle2 className="h-4 w-4" />Compte trouvé</div><p className="mt-1 h-12 text-base">Votre adresse existe déjà. Saisissez votre mot de passe pour confirmer définitivement votre participation.</p></div>
        ...
        {step === "new" ? <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); register.mutate({ ...form, email }); }}>
          <div className="rounded-xl border border-amber-100 bg-amber-50 p-4 text-sm leading-6 text-amber-950"><div className="flex items-center gap-2 font-bold"><UserPlus className="h-4 w-4" />Aucun compte trouvé</div><p className="mt-1 h-12 text-base">Complétez les informations suivantes pour créer votre compte. Vous devrez ensuite confirmer votre adresse par e-mail.</p></div>
          <div className="rounded-lg bg-slate-50 p-3 text-sm"><strong>{email}</strong><button type="button" className="ml-2 text-[#4A9B8E] underline" onClick={() => setStep("email")}>Modifier</button></div>
          <div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="first-name">Prénom</Label><Input id="first-name" required value={form.firstName} onChange={(event) => setForm({ ...form, firstName: event.target.value })} className="mt-1 h-12 text-base" /></div><div><Label htmlFor="last-name">Nom</Label><Input id="last-name" required value={form.lastName} onChange={(event) => setForm({ ...form, lastName: event.target.value })} className="mt-1 h-12 text-base" /></div></div>
          <div><Label htmlFor="phone">Téléphone</Label><Input id="phone" required value={form.phoneNumber} onChange={(event) => setForm({ ...form, phoneNumber: event.target.value })} placeholder="+212..." className="mt-1 h-12 text-base" /></div>
          <div><Label htmlFor="new-password">Mot de passe</Label><Input id="new-password" type="password" required minLength={8} value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} className="mt-1 h-12 text-base" /><p className="mt-1 text-xs text-slate-500">8 caractères minimum, dont une lettre majuscule.</p></div>
          <div><Label htmlFor="confirm-password">Confirmer le mot de passe</Label><Input id="confirm-password" type="password" required value={form.confirmPassword} onChange={(event) => setForm({ ...form, confirmPassword: event.target.value })} className="mt-1 h-12 text-base" /></div>
